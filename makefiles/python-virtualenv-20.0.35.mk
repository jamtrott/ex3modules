...
$(python-virtualenv)-builddeps = $(python) $(python-setuptools) $(python-setuptools_scm) $(python-importlib_metadata) $(python-six) $(python-filelock) $(python-distlib) $(python-appdirs) $(python-zipp)
...
$($(python-virtualenv)-prefix)/.pkginstall: $(modulefilesdir)/.markerfile $$(foreach dep,$$($(python-virtualenv)-builddeps),$(modulefilesdir)/$$(dep)) $($(python-virtualenv)-prefix)/.pkgcheck $($(python-virtualenv)-site-packages)/.markerfile
	cd $($(python-virtualenv)-srcdir) && \
		...
		$(PYTHON) -m pip install . --no-index --ignore-installed --prefix=$($(python-virtualenv)-prefix)
	@touch $@
