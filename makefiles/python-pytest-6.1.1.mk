...
$(python-pytest)-builddeps = $(python) $(python-atomicwrites) $(python-pluggy) $(python-wcwidth) $(python-importlib_metadata) $(python-packaging) $(python-py) $(python-more-itertools) $(python-attrs) $(python-zipp) $(python-pyparsing) $(python-six) $(python-toml) $(python-iniconfig) $(python-pip) $(python-wheel) $(python-setuptools)
...
$($(python-pytest)-prefix)/.pkginstall: $(modulefilesdir)/.markerfile $$(foreach dep,$$($(python-pytest)-builddeps),$(modulefilesdir)/$$(dep)) $($(python-pytest)-prefix)/.pkgcheck $($(python-pytest)-site-packages)/.markerfile
	cd $($(python-pytest)-srcdir) && \
		...
		$(PYTHON) -m pip install . --no-index --ignore-installed --prefix=$($(python-pytest)-prefix)
	@touch $@
